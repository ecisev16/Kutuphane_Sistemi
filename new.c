...
KITAPORNEK*  takeOrnek(KITAPORNEK **);
...
void takeKitaplar_file(KITAP **kitap){
    ...
    //KITAP *curr = *kitap;
    //currH->head = ktp->head;
    for(j=1;j<=ktp->adet;j++){

        //currH->head = ktp->head;
        new = takeOrnek(&ktp->head);
        //currH->head = ktp->head;
        ...
        strcpy(new->Durum,"rafta");
        sprintf(new->EtiketNo, "\n%s_%d", ktp->ISBN, j);
        //i=1;
        printf("%s", new->EtiketNo);
    }  
    //i = 1;
...
KITAPORNEK* takeOrnek(KITAPORNEK **kitapOrnek){
